...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.progress-bar-wrapper').addEventListener('click', function(e) {
        if(audio.duration) audio.currentTime = (e.offsetX / this.clientWidth) * audio.duration;
    });
    ...
    document.querySelectorAll('[id$="-btn"]').forEach(btn => {
        const modal = document.getElementById(btn.id.replace('-btn', '-modal'));
    ...
    document.getElementById('hide-controls-btn').addEventListener('click', () => {
        document.getElementById('controls-area').classList.add('hidden');
        document.getElementById('show-controls-btn').hidden = false;
    });
    document.getElementById('show-controls-btn').addEventListener('click', () => {
        ...
        document.getElementById('show-controls-btn').hidden = true;
    });

    // Settings
    document.querySelectorAll('#settings-modal input, #settings-modal select').forEach(el => {
        el.addEventListener('change', e => {
            const key = e.target.id.replace(/-/g, '_').replace('_picker', '').replace('smoothing', 'smoothing');
            visualizer.settings[key] = e.target.value;
            if (key === 'background_color') document.getElementById('visualizer-wrapper').style.backgroundColor = e.target.value;
