class Visualizer {
    constructor(canvas, centerImage) {
        this.canvas = canvas; this.centerImage = centerImage; this.ctx = this.canvas.getContext('2d');
        this.particles = []; this.isPlaying = false;
    ...
    renderFrame() {
        ...
        if (isRingType) {
            ...
            this.centerImage.style.width = '0px'; this.centerImage.style.height = '0px';
        }
        ...
        switch (this.settings.type) {
            case 'ring_bars': case 'ring_smooth': {
                ...
                for (let i = 0; i < barCount; i++) {
                    const barHeight = Math.pow(this.dataArray[i] / 255, this.settings.sensitivity) * maxVisualRadius * 0.5;
            ...
            case 'linear_mirror_bars': case 'linear_mirror_smooth': {
                ...
                for (let i = 0; i < barCount; i++) {
                    ...
                    if (this.settings.type === 'linear_mirror_bars') { this.ctx.fillRect(x, y, barWidth * 0.9, barHeight); } 
            ...
            case 'monstercat': {
                ...
                for (let i = 0; i < barCount; i++) {
                    ...
                    if (i < 10 && this.dataArray[dataIndex] > 200 && Math.random() > 0.5) { this.spawnParticles(x, centerY, 3); }
                }
                break;
            }
        }
    ...
    spawnParticles(x, y, amount) {
        for (let i = 0; i < amount; i++) { this.particles.push({ x: x, y: y, vx: (Math.random() - 0.5) * 3, vy: (Math.random() - 0.5) * 3, life: 1, alpha: 1 }); }
    }
    updateAndDrawParticles(width, height) {
        for (let i = this.particles.length - 1; i >= 0; i--) {
            const p = this.particles[i]; p.x += p.vx; p.y += p.vy; p.life -= 0.02;
            if (p.life <= 0 || p.x < 0 || p.x > width || p.y < 0 || p.y > height) { this.particles.splice(i, 1); } 
            else { this.ctx.globalAlpha = p.life; this.ctx.fillRect(p.x, p.y, 2, 2); this.ctx.globalAlpha = 1; }
        }
    }
}
